...
def check_ollama():
    ...
    if result.returncode != 0:
        ...
        warn("Please install it from https://ollama.ai and run: ollama pull mistral")
...
def configure_env():
  ...
  improve the experience:
    ...
    ElevenLabs  — High-quality voice output (free tier available)
                  https://elevenlabs.io

    SerpAPI     — Real-time web search (free tier available)
                  https://serpapi.com
  ...
  Keys are saved to a local .env file and never leave your machine.
    ...
    if confirm("Do you have an ElevenLabs API key?"):
        ...
        if key:
            ...
            voice_id = ask("ElevenLabs Voice ID (press Enter for default 'George')").strip()
            if voice_id:
                env_lines.append(f"ELEVENLABS_VOICE_ID={voice_id}")
            else:
                env_lines.append("ELEVENLABS_VOICE_ID=JBFqnCBsd6RMkjVDRZzb")
...
def done():
    ...
    print("""
  Friday is ready to run. Start it with:

      python friday.py

  Optional flags:

      --dev          Verbose logging + text input mode
      --offline-tts  Force local TTS, skip ElevenLabs
      --no-search    Disable web search
      --voice        Play audio responses in text mode
...
if __name__ == "__main__":
  ...
  ╚═╝     ╚═╝  ╚═╝╚═╝╚═════╝ ╚═╝  ╚═╝   ╚═╝  
    """)
    print("  First-time setup for Friday v0.68")
    print("  ------------------------------------")
    print("  Assumes: Python, Ollama, and Mistral are already installed.")
    print("  See README.md if you haven't set those up yet.\n")
